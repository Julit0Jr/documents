...
app.post('/deposit', requireSignedIn, function(req, res) {
	...
	User.findOne({ where: { email: owner } }).then(function(withdrawer) {
		Account.findOne({ where: { user_id: withdrawer.id } }).then(function(withdrawerAccount) {
			database.transaction(function(t){
				return withdrawerAccount.update({
					balance: withdrawerAccount.balance + amount
				}, {transaction: t});
			}).then(function() {
					req.flash('statusMessage', 'Deposit amount of ' + amount + ' complete!');
					res.redirect('/profile');
			});
		});
	});
		...
		Account.findOne({ where: { user_id: withdrawer.id } }).then(function(withdrawerAccount) {
			database.transaction(function(t){
				return withdrawerAccount.update({
					...
					req.flash('statusMessage', 'Withdraw amount of ' + amount + ' complete!');
					res.redirect('/profile');
			});
		});
	});
